...
6. 打开创作者主页或代表内容寻找参考。
7. 女生妆容结果页可参与 Plus 购买意向测试；只选择场景、妆造方向和意向，不会收费或调用 Plus AI。
...
女生妆容结果页会在三位免费参考完整展示后，提供一次性 Plus 方案的购买意向测试。页面按匿名标签页会话固定展示 9.9 元、19.9 元或 29.9 元，方案包括详细面部结构报告、多场景妆容方案和按方案发现公开美妆博主。

这不是实际购买：不会扣款，不收集支付资料、联系方式、照片或面部数据，也不会调用 Plus AI。场景与妆造方向只用于当前页面交互；统计服务只接收固定事件、价格分组、随机会话标识和时间。详细实验口径见 [Plus 付费意向实验](docs/PLUS_PAID_INTENT_EXPERIMENT.md)。
