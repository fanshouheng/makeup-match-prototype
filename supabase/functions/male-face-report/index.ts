...
function buildMessages(input: ReportRequest) {
  const modeInstruction = input.mode === "roast"
    ? "生成明确偏负面的锐评：每条评论都要从比例相对关系出发挑刺，可以刻薄并使用轻度贬义比喻；禁止正向评价、先贬后夸、转折挽尊，以及用有神、高级、耐看、利落、可爱、帅等词暗夸。只能吐槽照片中的视觉结构，不得把比例解释为性格、品德、智力、能力或处事方式；不得使用算计、心眼、气量、刻薄相、精明、不吃亏、不好糊弄等人格词。仍不得羞辱、暗示残疾或攻击无法改变的身体特征。"
